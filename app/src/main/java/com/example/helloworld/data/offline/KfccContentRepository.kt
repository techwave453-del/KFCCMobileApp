...
class KfccContentRepository(
    ...
    suspend fun getMedia(): List<MediaItem> {
        ...
        return runCatching {
            ...
            db.mediaItemDao().upsertAll(rows.map(::toMediaEntity))
            rows
        }.getOrElse { emptyList() }
    }
    ...
    private fun toMediaEntity(x: MediaItem) = MediaItemEntity(
        x.id, null, x.title, x.type, x.category, x.description, x.url, null,
        x.createdAt, true, null, x.featured
    )

    private fun toMedia(x: MediaItemEntity) = MediaItem(
        x.id, x.title, x.type, x.category, x.description, x.url, x.featured, x.createdAt
    )

    private fun toEventEntity(x: EventItem) = EventEntity(
        x.id, x.slug, x.title, x.category, x.shortDescription, x.description, x.image,
        x.flyerUrl, x.startAt, x.endAt, x.allDay, x.location, x.address, x.attendanceType,
        x.registrationUrl, x.contact, x.livestreamUrl, x.featured, x.status, x.displayOrder,
    ...
    private fun toNotificationEntity(x: AppNotification) = NotificationEntity(
        x.id, x.userId, x.title, x.message, x.type, x.createdAt
    )
