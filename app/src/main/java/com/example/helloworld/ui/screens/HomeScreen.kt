    ...
    LazyColumn(
        ...
        item {
            ChurchHero(
                ...
                imageUrl = info.services.firstOrNull()?.imageUrl
            )
        }
        item {
        ...
        if (events.isNotEmpty()) {
            item {
                SectionHeader("Upcoming Events", modifier = Modifier.padding(horizontal = 18.dp), action = "View all")
            }
                ...
                Card(Modifier.fillMaxWidth().padding(horizontal = 18.dp, vertical = 6.dp), shape = androidx.compose.foundation.shape.RoundedCornerShape(18.dp)) {
                    Column(Modifier.padding(16.dp)) {
                        ...
                        if (event.short_description.isNotBlank()) {
                            ...
                            Text(event.short_description, style = MaterialTheme.typography.bodySmall, maxLines = 2)
                        }
                    }
                }
            }
        ...
        item {
            SectionHeader("Join Us In Worship", modifier = Modifier.padding(start = 18.dp, end = 18.dp, top = 18.dp, bottom = 4.dp), action = "All services")
        }
        items(info.services) { service ->
            ChurchServiceCard(service = service, modifier = Modifier.padding(horizontal = 18.dp, vertical = 7.dp))
        }
    }
}
