    ...
    Column(
        ...
        Row(Modifier.fillMaxWidth()) {
            Column(Modifier.weight(1f)) {
                ...
                Text(
                    "Manage public homepage content directly in Supabase.",
            ...
            IconButton(
                ...
                Icon(Icons.Default.Refresh, "Refresh website content")
            }
        }

        Text(
            "Changes are written directly to the Supabase site_content table.",
        ...
        if (canEditIdentity) {
            Text("Protected contact details", style = MaterialTheme.typography.titleMedium)
        ...
        if (canManageLive) {
            ...
            Field(
                ...
                { viewModel.update(content.copy(liveStream = content.liveStream.copy(title = it))) },
                saving
            )
            Field(
                "YouTube Live URL",
                content.liveStream.url,
                { viewModel.update(content.copy(liveStream = content.liveStream.copy(url = it))) },
                saving
            )
            Field(
                "Live description",
                content.liveStream.description,
                { viewModel.update(content.copy(liveStream = content.liveStream.copy(description = it))) },
                saving,
                3
            )
            Row(
                ...
                Switch(
                    checked = content.liveStream.enabled,
                    onCheckedChange = {
                        viewModel.update(
                            content.copy(
                                liveStream = content.liveStream.copy(enabled = it)
                            )
                        )
                    },
                    enabled = !saving
        ...
        Button(
            onClick = { viewModel.save(canEditIdentity, canManageLive) },
            enabled = !loading && !saving,
            modifier = Modifier.fillMaxWidth()
        ) {
            if (saving) CircularProgressIndicator(modifier = Modifier.size(20.dp))
            else Text("Save Website Content")
        }
    }
}
